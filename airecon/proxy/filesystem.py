...
_LINE_COUNT_EXTENSIONS = {".txt", ".csv", ".out", ".log", ".nmap", ".md", ".json", ".xml", ".html", ".htm", ".sh", ".py"}
_MAX_DEPTH = 3
    ...
    try:
        ...
        if os.path.isabs(path) and os.path.isfile(path):
            ...
            if not (is_in_workspace or is_in_project):
                return {"success": False, "error": f"Access denied: Absolute path {path} is outside the allowed sandbox."}
...
def _read_with_pagination(file_path: Path, offset: int, limit: int) -> dict[str, Any]:
    ...
    if has_more:
        ...
        meta_parts.append(f"[More lines available — read next page with: offset={next_offset}, limit={limit}]")
...
def list_files(path: str = "") -> dict[str, Any]:
    ...
    try:
        ...
        base_dir = (workspace_root / clean_path).resolve() if clean_path else workspace_root
