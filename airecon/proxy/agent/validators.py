...
# Pre-compiled regex for PoC description HTTP evidence check
_HTTP_EVIDENCE_RE = re.compile(
    r"(http\s+[2345]\d{2}|status[:\s]+[2345]\d{2}|\b[2345]\d{2}\s+(ok|found|forbidden|"
    r"redirect|not found|created|accepted|no content|moved|unauthorized|bad request|"
    r"internal server error)\b|response[:\s]+[2345]\d{2}|→\s*[2345]\d{2}|"
    r"returned\s+[2345]\d{2}|returns\s+[2345]\d{2}|got\s+[2345]\d{2}|"
    r"observed[:\s]+[2345]\d{2})",
    re.IGNORECASE,
)
...
class _ValidatorMixin:
    ...
    def _validate_tool_args(
        ...
        if tool_name == "execute":
            ...
            # Block writing security reports as markdown files — must use create_vulnerability_report
            ...
            if path_lower.endswith(".md") and any(r in path_lower for r in _REPORT_NAMES):
                return False, (
            ...
            # Reject unverified/speculative findings — all reports must be confirmed
            UNVERIFIED_PHRASES = (
            ...
            # poc_script_code must reference a real URL (not a generic template)
