...
_SESSIONS_INDEX = SESSIONS_DIR / "sessions_index.json"

# Similarity threshold for vulnerability deduplication (0-1)
VULN_SIMILARITY_THRESHOLD = 0.7

# Module-level compiled regexes for update_from_parsed_output (hot path)
...
def _is_duplicate_vulnerability(
    ...
    threshold: float = VULN_SIMILARITY_THRESHOLD,
) -> bool:
    """Check if a vulnerability is a duplicate of an existing one.

    The `threshold` should be passed in by the caller (loaded once from config)
    rather than re-fetching config on every invocation.
    """
...
def _update_sessions_index(session: SessionData) -> None:
    """Upsert one session's summary into sessions_index.json."""
    try:
        index: dict[str, dict] = {}
        if _SESSIONS_INDEX.exists():
            try:
                index = json.loads(_SESSIONS_INDEX.read_text(encoding="utf-8"))
            except Exception:
                index = {}
        ...
        _SESSIONS_INDEX.write_text(
            json.dumps(index, indent=2, default=str), encoding="utf-8"
        )
    except Exception as e:
        logger.warning(f"Failed to update sessions index: {e}")
...
def save_session(session: SessionData) -> None:
    ...
    try:
        ...
        # Keep the lightweight index in sync for fast list_sessions() reads
        _update_sessions_index(session)
        logger.info(f"Saved session {session.session_id} (target={session.target})")
    except Exception as e:
...
def list_sessions() -> list[dict]:
    ...
    Fast path: read the pre-built sessions_index.json (single file read).
    Fallback: full directory scan (backward-compat with sessions saved before the index existed).
    """
    ...
    # --- Slow path: read each session file (legacy, also rebuilds the index) ---
    sessions: list[dict] = []
    for path in SESSIONS_DIR.glob("*.json"):
        if path.name == "sessions_index.json":
            continue
        try:
    ...
    # Opportunistically write the index so next call is fast
    try:
        index_data = {s["session_id"]: s for s in sessions}
        _SESSIONS_INDEX.write_text(
...
def update_from_parsed_output(
    ...
    if parsed.technologies:
        for name, version in parsed.technologies.items():
            if name and name not in session.technologies:
                ...
                # Upgrade an empty-version entry when a version is now available
                session.technologies[name] = version

    if not parsed.items:
        return

    # Load dedup threshold once for this batch (not per-vuln)
    try:
        from ..config import get_config
        _dedup_threshold = get_config().vuln_similarity_threshold
    except Exception:
        _dedup_threshold = VULN_SIMILARITY_THRESHOLD

    # Regexes are module-level compiled constants (not re-compiled here)
    for item in parsed.items:
        ...
        if _SEVERITY_RE.match(item_stripped):
            ...
            if not _is_duplicate_vulnerability(new_vuln, session.vulnerabilities, _dedup_threshold):
...
def session_to_context(session: SessionData) -> str:
    ...
    parts = [f"[SYSTEM: PREVIOUS SESSION DATA — Session {session.session_id} for {target_label}]"]
