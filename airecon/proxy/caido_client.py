...
class CaidoClient:
    ...
    async def gql(
        ...
        Automatically refreshes the session token on 401 and retries once,
        so Caido restarts or token expiry are handled transparently.
        """
            ...
            try:
                async with httpx.AsyncClient(timeout=30.0) as c:
                    ...
                    if resp.status_code == 401 and attempt == 0:
                        # Token rejected — clear cache and retry with a fresh one
                        logger.warning("Caido returned 401 — clearing cached token and retrying")
                        cls._token = None
                        continue
                    return resp.json()
            except Exception as e:
        ...
        return {"errors": [{"message": "Caido authentication failed after token refresh"}]}
    ...
    def find_fuzz_offsets(cls, raw_http: str) -> list[dict[str, int]]:
        ...
        while True:
            ...
            if idx == -1:
                break
            # Position in clean bytes = original position minus bytes already removed
