...
class RealTimeEvent:
    ...
    event_type: str                # "finding", "progress", "chain_discovered", "complete", "error"
    data: dict[str, Any]
    ...
    async def _fuzz_single(
        ...
        async with self._semaphore:
            ...
            try:
                ...
                logger.debug(f"Fuzz request error param={param} payload={payload!r}: {exc}")
        return None
    ...
    def generate_wordlist_combinations(
        ...
        for suffix in _PRIVILEGE_SUFFIXES + _ENV_SUFFIXES + _DEMO_SUFFIXES:
            for word in base_words:
                if not _add(f"{word}_{suffix}"): return combinations
                if not _add(f"{word}{suffix}"): return combinations
                if not _add(f"{suffix}_{word}"): return combinations
        ...
        for word in base_words:
            for suffix in _PRIVILEGE_SUFFIXES[:5] + ["Id", "Key", "Token", "Secret"]:
                ...
                if not _add(camel): return combinations

        # 5. Two-word combinations with _ and - separators
        for w1 in base_words:
            for w2 in base_words:
                if w1 != w2:
                    if not _add(f"{w1}_{w2}"): return combinations
                    if not _add(f"{w1}-{w2}"): return combinations
...
class ExpertHeuristics:
    ...
    def analyze_response(response: str) -> dict[str, Any]:
        ...
        # Code execution indicators (separate category, no overlap with sql_error)
        code_hit = False
    ...
    def analyze_response_differential(
        ...
        if payload and len(payload) > 3 and payload in fuzz_body:
            if payload not in baseline_body:
                ...
                if any(tag in payload for tag in ["<script", "<img", "<svg", "javascript:"]):
        ...
        if fuzz_status != baseline_status:
            if baseline_status == 200 and fuzz_status == 500:
                confidence += 0.4
                evidence.append(f"Status change 200→500 (server error on payload)")
            elif fuzz_status == 403:
                confidence += 0.1
                evidence.append("WAF/403 triggered by payload")
            elif baseline_status == 200 and fuzz_status in (301, 302):
                confidence += 0.3
                evidence.append(f"Redirect triggered (possible open redirect/SSRF): {fuzz_status}")
        ...
        if baseline_len > 0:
            ...
            if delta_ratio > 0.5 and fuzz_len > baseline_len + 200:
                confidence += 0.25
                evidence.append(f"Response significantly larger (+{fuzz_len - baseline_len}B) — possible data leak")
            elif delta_ratio < -0.5 and baseline_len > 200:
                confidence += 0.15
                evidence.append("Response significantly smaller — possible truncation/filter")
        ...
        for pattern in VULNERABLE_PATTERNS["sql_error"]:
            if pattern in fuzz_lower and pattern not in baseline_lower:
                confidence += 0.5
                evidence.append(f"SQL error in fuzz response (not in baseline): {pattern!r}")
        ...
        for sig in lfi_signatures:
            if sig in fuzz_lower and sig not in baseline_lower:
                confidence += 0.8
                evidence.append(f"LFI/path traversal signature in fuzz response: {sig!r}")
        ...
        for waf_name, sig in WAF_SIGNATURES.items():
            ...
            for h in sig["headers"]:
                if h in headers_lower:
                    return waf_name
            # Status code + body pattern match requires body, check status at least
            if status_code in sig["status_codes"]:
                # Tentative match on status alone — caller should verify body too
                for h in sig["headers"]:
...
class ExploitChainEngine:
    ...
    def prioritize_chains(self, chains: list[ExploitChain]) -> list[ExploitChain]:
        ...
        def _sort_key(c: ExploitChain) -> tuple[int, float]:
            sev_score = _SEVERITY_ORDER.index(c.combined_severity) if c.combined_severity in _SEVERITY_ORDER else 0
    ...
    def _compute_chain_severity(steps: list[ChainLink]) -> str:
        ...
        critical_types = {"rce", "rce_via_outfile", "rce_via_log_poison", "reverse_shell", "data_exfiltration"}
...
class InteractiveRealTimeTester:
    ...
    async def stream_fuzz(
        ...
        yield RealTimeEvent(
            event_type="progress",
            data={"phase": "baseline", "params": params, "message": "Probing baseline responses..."},
        )
        ...
        yield RealTimeEvent(
            event_type="progress",
            data={"phase": "fuzzing", "total_tests": total, "message": f"Starting {total} fuzz tests..."},
        )
        ...
        if self._findings:
            yield RealTimeEvent(
                event_type="progress",
                data={"phase": "chaining", "message": f"Discovering exploit chains from {len(self._findings)} findings..."},
            )
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "chains": [
                {"name": c.name, "severity": c.combined_severity, "confidence": c.total_confidence}
...
async def quick_fuzz_url(url: str, params: list[str] | None = None) -> list[FuzzResult]:
