...
def create_vulnerability_report(
    ...
    # active target is target.com, save the report under target.com/vulnerabilities/
    if _active_target:
        active_clean = str(_active_target).replace("https://", "").replace("http://", "").split("/")[0]
        active_clean = re.sub(r'[^a-zA-Z0-9\.\-_]', '_', active_clean)
        if target_clean != active_clean and (
            target_clean.endswith("." + active_clean) or active_clean.endswith("." + target_clean)
        ):
