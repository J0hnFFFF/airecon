...
async def _create_browser() -> Browser:
    ...
    try:
        _state.browser = await _state.playwright.chromium.connect_over_cdp("http://localhost:9222", timeout=3000)
    except Exception as e:
        logger.warning(f"Could not connect to browser in Docker Sandbox (port 9222). Falling back to local host browser. Error: {e}")
        try:
            ...
            if _state.playwright:
                await _state.playwright.stop()
                _state.playwright = None
            raise RuntimeError(f"Failed to launch fallback local browser: {e2}")
...
class BrowserInstance:
    ...
    def _resolve_tab_id(self, tab_id: str | None) -> str:
        ...
        if tab_id and tab_id not in self.pages:
            logger.warning(f"Tab '{tab_id}' not found — falling back to current tab '{self.current_page_id}'")
    ...
    async def _setup_console_logging(self, page: Page, tab_id: str) -> None:
        ...
        async def handle_response(response: Any) -> None:
            ...
            if any(t in content_type for t in ("json", "text/plain", "javascript", "xml", "html")):
                try:
    ...
    async def _get_page_state(self, tab_id: str | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            if len(text_content) > 3000:
                text_content = text_content[:3000] + "... [TRUNCATED, use execute_js or view_source for more]"
        except Exception:
    ...
    async def _close_tab(self, tab_id: str) -> dict[str, Any]:
        ...
        if tab_id in self.network_requests: del self.network_requests[tab_id]
    ...
    async def _get_network_logs(self, tab_id: str | None = None, clear: bool = False) -> dict[str, Any]:
        tab_id = self._resolve_tab_id(tab_id)
        reqs = self.network_requests.get(tab_id, [])
        if clear: self.network_requests[tab_id] = []
        state = await self._get_page_state(tab_id)
    ...
    def get_network_logs(self, tab_id: str | None = None, clear: bool = False) -> dict[str, Any]:
        with self._execution_lock: return self._run_async(self._get_network_logs(tab_id, clear))
    ...
    async def _view_source(self, tab_id: str | None = None) -> dict[str, Any]:
        ...
        state["full_page_source"] = full_source  # used by executor for file saving, stripped before LLM context
        return state
    ...
    def login_form(
        ...
        with self._execution_lock:
            return self._run_async(
                self._login_form(url, username, password, username_selector, password_selector, submit_selector, tab_id)
            )
    ...
    async def _login_form(
        ...
        if not username_filled:
            logger.warning(f"login_form: no username selector matched ({username_selector}). Auth may fail.")
        ...
        if not password_filled:
            logger.warning(f"login_form: no password selector matched ({password_selector}). Auth may fail.")
        await asyncio.sleep(0.3)
        # Submit
        submitted = False
    ...
    def handle_totp(
        ...
        with self._execution_lock:
            return self._run_async(self._handle_totp(totp_secret, field_selector, tab_id))

    async def _handle_totp(self, totp_secret: str, field_selector: str, tab_id: str | None) -> dict[str, Any]:
        try:
    ...
    def inject_cookies(self, cookies: list[dict[str, Any]], tab_id: str | None = None) -> dict[str, Any]:
        with self._execution_lock:
            return self._run_async(self._inject_cookies(cookies, tab_id))

    async def _inject_cookies(self, cookies: list[dict[str, Any]], tab_id: str | None) -> dict[str, Any]:
        if not self.context:
            raise ValueError("Browser not launched")
    ...
    def oauth_authorize(
        ...
        with self._execution_lock:
            return self._run_async(self._oauth_authorize(oauth_url, callback_prefix, tab_id))

    async def _oauth_authorize(self, oauth_url: str, callback_prefix: str, tab_id: str | None) -> dict[str, Any]:
        if not self.pages:
...
class BrowserTabManager:
        ...
        try:
            ...
            if is_crash and self._restart_count < 5:
                ...
                try:
                    ...
                    if method is None:
                        return {"error": f"Browser crashed and could not rebind method '{fn.__name__}' after restart"}
    ...
    def click(self, coordinate: str, tab_id: str | None = None) -> dict[str, Any]:
        result = self._safe_action("click", self._ensure_launched().click, coordinate, tab_id)
        result.setdefault("message", f"Clicked at {coordinate}")
        return result

    def type_text(self, text: str, tab_id: str | None = None) -> dict[str, Any]:
        result = self._safe_action("type", self._ensure_launched().type_text, text, tab_id)
        result.setdefault("message", "Typed text")
        return result

    def scroll(self, direction: str, tab_id: str | None = None) -> dict[str, Any]:
        result = self._safe_action("scroll", self._ensure_launched().scroll, direction, tab_id)
    ...
    def switch_tab(self, tab_id: str) -> dict[str, Any]:
        result = self._safe_action("switch_tab", self._ensure_launched().switch_tab, tab_id)
        result.setdefault("message", f"Switched to tab {tab_id}")
        return result

    def close_tab(self, tab_id: str) -> dict[str, Any]:
        result = self._safe_action("close_tab", self._ensure_launched().close_tab, tab_id)
        result.setdefault("message", f"Closed tab {tab_id}")
        return result

    def wait_browser(self, duration: float, tab_id: str | None = None) -> dict[str, Any]:
        result = self._safe_action("wait", self._ensure_launched().wait, duration, tab_id)
        result.setdefault("message", f"Waited {duration}s")
        return result

    def execute_js(self, js_code: str, tab_id: str | None = None) -> dict[str, Any]:
        result = self._safe_action("execute_js", self._ensure_launched().execute_js, js_code, tab_id)
        result.setdefault("message", "JavaScript executed successfully")
        return result

    def double_click(self, coordinate: str, tab_id: str | None = None) -> dict[str, Any]:
        result = self._safe_action("double_click", self._ensure_launched().double_click, coordinate, tab_id)
        result.setdefault("message", f"Double clicked at {coordinate}")
        return result

    def hover(self, coordinate: str, tab_id: str | None = None) -> dict[str, Any]:
        result = self._safe_action("hover", self._ensure_launched().hover, coordinate, tab_id)
        result.setdefault("message", f"Hovered at {coordinate}")
        return result

    def press_key(self, key: str, tab_id: str | None = None) -> dict[str, Any]:
        result = self._safe_action("press_key", self._ensure_launched().press_key, key, tab_id)
        result.setdefault("message", f"Pressed key {key}")
        return result

    def save_pdf(self, file_path: str, tab_id: str | None = None) -> dict[str, Any]:
        result = self._safe_action("save_pdf", self._ensure_launched().save_pdf, file_path, tab_id)
        result.setdefault("message", f"Page saved as PDF: {file_path}")
        return result

    def get_console_logs(self, tab_id: str | None = None, clear: bool = False) -> dict[str, Any]:
        result = self._safe_action("get_console_logs", self._ensure_launched().get_console_logs, tab_id, clear)
        result.setdefault("message", "Console logs retrieved")
        return result

    def get_network_logs(self, tab_id: str | None = None, clear: bool = False) -> dict[str, Any]:
        result = self._safe_action("get_network_logs", self._ensure_launched().get_network_logs, tab_id, clear)
        result.setdefault("message", "Network logs retrieved")
        return result

    def view_source(self, tab_id: str | None = None) -> dict[str, Any]:
        result = self._safe_action("view_source", self._ensure_launched().view_source, tab_id)
    ...
    def save_auth_state(self) -> dict[str, Any]:
        result = self._safe_action("save_auth_state", self._ensure_launched().save_auth_state)
    ...
    def oauth_authorize(self, oauth_url: str, callback_prefix: str = "", tab_id: str | None = None) -> dict[str, Any]:
        result = self._safe_action(
            "oauth_authorize",
...
def browser_action(
    ...
    try:
        ...
        elif action == "get_network_logs": return _manager.get_network_logs(tab_id, clear)
        ...
        # ── Auth actions ──────────────────────────────────────────────────────
            ...
            return _manager.login_form(url, username, password, username_selector, password_selector, submit_selector, tab_id)
