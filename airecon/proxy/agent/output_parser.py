...
class ParsedOutput:
    ...
    # Technology fingerprints extracted from tool output: {"nginx": "1.18.0", "Bootstrap": "3.3.7"}
    ...
    for line in stdout.strip().split("\n"):
        ...
        if line.startswith("{"):
            try:
                ...
                # tech can be a list of "Name/version" strings (httpx -tech-detect)
                ...
                if tech_raw:
                    ...
                    for t in tech_raw:
                        # Normalize "Name/version" → technologies["Name"] = "version"
                        if "/" in t:
        ...
        if not found_any:
            for bare_m in _tech_bare_re.finditer(summary_str):
                ...
                if name not in {"HTTP", "HTML", "URL", "API"}:
                    technologies[name] = ""
                    items.append(name + (f" on {current_target}" if current_target else ""))
