...
def _detect_model_capabilities(model_name: str) -> tuple[bool, bool]:
    ...
    supports_thinking = any(pattern in model_lower for pattern in THINKING_MODELS)
...
class OllamaClient:
    ...
    def __init__(self, base_url: str | None = None, model: str | None = None) -> None:
        ...
        if self._supports_thinking or self._supports_native_tools:
            detected_think, detected_tools = _detect_model_capabilities(self.model)
        ...
        logger.info(
            f"Initializing Ollama SDK client for host: {host}, model: {self.model}, timeout: {cfg.ollama_timeout}s"
        )
        logger.info(
            f"Model capabilities: thinking={self._supports_thinking}, native_tools={self._supports_native_tools}"
        )
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    ...
                    continue
                logger.error(f"Ollama ResponseError after {max_retries + 1} attempts: {e.error}")
                raise
