...
async def lifespan(app: FastAPI):
    ...
    engine = DockerEngine()
    agent = AgentLoop(ai=ai_client, engine=engine)

    # Check AI connectivity (if provider supports it/simple health check)
    # Most cloud providers don't need a local health check, but we check config
    logger.info(f"  AI Provider: {cfg.ai_provider} ({getattr(cfg, f'{cfg.ai_provider}_model', cfg.ollama_model)})")
    ...
    try:
        ...
        logger.error(f"Agent initialization failed: {e}. Marking agent unavailable.")
...
async def get_status() -> JSONResponse:
    ...
    ai_ok = await ai_client.health_check() if ai_client else False
    docker_ok = engine.is_connected if engine else False

    cfg = get_config()
    provider = cfg.ai_provider.lower()

    # Build provider-specific info block
    if provider == "ollama":
        ai_info = {
            "connected": ai_ok,
            "provider": "ollama",
        ...
        # Cloud providers: connection is always assumed OK (errors surface at inference time)
        model_field = f"{provider}_model"
        ai_info = {
            "connected": True,
            "provider": provider,
            "model": getattr(cfg, model_field, "unknown"),
        }
        ai_ok = True  # Don't degrade status just because cloud AI has no local health check

    return JSONResponse({
        "status": "ok" if (ai_ok and docker_ok) else "degraded",
        "ai": ai_info,
        # Keep legacy "ollama" key for UI backward compatibility
        "ollama": ai_info,
        "docker": {
            "connected": docker_ok,
...
async def list_tools() -> JSONResponse:
    ...
    if agent and agent.tools:
        return JSONResponse({"count": len(agent.tools), "tools": agent.tools})

    # Agent not yet initialized — fall back to raw engine tool discovery
    if not engine:
...
async def _stream_agent_events(message: str) -> AsyncIterator[dict]:
    ...
    if not agent:
        yield {"event": "error", "data": json.dumps({"type": "error", "message": "Agent not initialized"})}
        return
