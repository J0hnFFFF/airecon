...
class AgentState:
    ...
    tool_counts: dict[str, int] = field(default_factory=lambda: {"exec": 0, "total": 0, "subagents": 0})
    ...
    async def compress_with_llm(self, ai_client: Any, keep_recent: int = 30) -> None:
        ...
        for i in range(0, len(to_compress), CHUNK_SIZE):
            ...
            try:
                summary_text = await ai_client.complete(prompt)
                summaries.append({
                    "role": "system",
                    "content": (
                ...
                logger.warning(f"Memory compression LLM call failed, keeping original: {e}")
