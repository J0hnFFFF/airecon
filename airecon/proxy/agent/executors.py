...
class _ExecutorMixin:
    ...
    async def _execute_local_browser_tool(
        ...
        try:
            ...
            # Auto-save view_source / get_console_logs / get_network_logs to workspace
            if success and self.state.active_target:  # type: ignore[attr-defined]
                action = arguments.get("action")
                if action in ("view_source", "get_console_logs", "get_network_logs"):
                    try:
                        inner = result.get("result", {})
                        if not isinstance(inner, dict):
                            inner = {}
                        page_url = inner.get("url", "")
                        netloc = urlparse(page_url).netloc.replace(":", "_") if page_url else "unknown"
                        ...
                        if action == "view_source":
                            # Use full_page_source for file (untruncated), fall back to page_source
                            source_full = inner.get("full_page_source") or inner.get("page_source", "")
                            ...
                            js_src = re.findall(r'src=["\']([^"\']*\.js[^"\']*)["\']', source_full)
                            js_abs = re.findall(r'https?://[^\s"\'<>]+\.js(?:\?[^\s"\'<>]*)?', source_full)
                            ...
                            inner = dict(inner)
                            inner.pop("full_page_source", None)  # strip from LLM context
                            ...
                            console_file = host_output / f"console_{domain}.txt"
                            with open(console_file, "w", encoding="utf-8") as f:
                                f.write(f"URL: {page_url}\nLogs captured: {len(logs)}\n{'='*60}\n")
                                for log in logs:
                                    f.write(f"[{log.get('type', 'log')}] {log.get('text', '')}\n")
                            ...
                            with open(net_file, "w", encoding="utf-8") as f:
                                ...
                                for entry in reqs:
                                    ...
                                    url = entry.get("url", "")
                                    if etype == "request":
                                        f.write(f">> {entry.get('method','GET')} [{entry.get('resource_type','')}] {url}\n")
                                        if entry.get("post_data"):
                                            f.write(f"   BODY: {entry['post_data']}\n")
                                    elif etype == "response":
                                        f.write(f"<< {entry.get('status','')} {url}  [{entry.get('content_type','')}]\n")
                                        if entry.get("body"):
                            ...
                            inner = dict(inner)
                            inner.pop("network_requests", None)  # strip raw list from LLM context
                            inner["auto_saved"] = saved_path
                            inner["note"] = (
                        ...
                        logger.warning(f"Failed to auto-save browser result: {_e}")
        ...
        if success and self.state.active_target:  # type: ignore[attr-defined]
            try:
                ...
                with open(dork_file, "a", encoding="utf-8") as f:
                    f.write(entry)
                saved_path = f"output/dork_results.txt"
    ...
    async def _execute_report_tool(
        ...
        try:
            result = await asyncio.to_thread(
                ...
                **arguments,
                _active_target=self.state.active_target,  # type: ignore[attr-defined]
            )
            success = result.get("success", False)
            try:
                self._save_tool_output(tool_name, arguments, result)  # type: ignore[attr-defined]
            ...
            # Mark matching vulnerability as reported so REPORT phase can transition
            if success and self._session:  # type: ignore[attr-defined]
                report_title = arguments.get("title", "")
                matched = False
                for vuln in self._session.vulnerabilities:  # type: ignore[attr-defined]
    ...
    async def _execute_advanced_fuzz_tool(
        ...
        vuln_types = arguments.get("vuln_types", ["sql_injection", "xss", "idor", "ssti", "command_injection", "path_traversal", "xxe", "race_condition", "parameter_pollution", "mass_assignment"])
        ...
        try:
            ...
            if not results:
                res_dict = {"success": True, "result": "No vulnerabilities found with confidence > 0.60."}
            else:
                findings_list = []
                for r in results:
                    findings_list.append(f"Param: {r.parameter} | Vuln: {r.vuln_type} | Severity: {r.severity} | Conf: {r.confidence:.2f} | Evidence: {r.evidence}")
                res_dict = {"success": True, "findings": findings_list}
            
            try:
                self._save_tool_output(tool_name, arguments, res_dict)  # type: ignore[attr-defined]
            except Exception:
                pass
            success = True
        except Exception as e:
    ...
    async def _execute_quick_fuzz_tool(
        ...
        try:
            ...
            if not results:
                ...
                findings_list = [
                    f"Param: {r.parameter} | Vuln: {r.vuln_type} | Severity: {r.severity} | Conf: {r.confidence:.2f} | Evidence: {r.evidence}"
                    for r in results
                ]
                res_dict = {"success": True, "findings": findings_list, "total": len(findings_list)}
        ...
        try:
            ...
            for f in getattr(tester, "_findings", []):
                findings_list.append(
                    f"Param: {f.parameter} | Vuln: {f.vuln_type} | Severity: {f.severity} | Conf: {f.confidence:.2f} | Evidence: {f.evidence}"
                )
    ...
    async def _execute_generate_wordlist_tool(
        ...
        self.state.tool_counts["total"] += 1  # type: ignore[attr-defined]
        return success, duration, res_dict, self._last_output_file  # type: ignore[attr-defined]
    ...
    async def _execute_run_parallel_agents_tool(
        ...
        try:
            runner = ParallelAgentRunner(engine=self.engine)  # type: ignore[attr-defined]
            results = await runner.run_parallel(targets, prompt)

            # Summarize the vulnerabilities found per target
            res_dict = {"success": True, "results": {t: len(s.vulnerabilities) for t, s in results.items()}}
    ...
    async def _execute_caido_list_requests_tool(
        ...
        try:
            ...
            if "errors" in data:
                ...
                res_dict = {"success": True, "requests": requests, "total": len(requests)}
                success = True
        except Exception as e:
    ...
    async def _execute_caido_send_request_tool(
        ...
        try:
            ...
            res_dict = {"success": False, "error": "Caido did not respond within 60 seconds"}
            success = False
        except Exception as e:
    ...
    async def _execute_caido_automate_tool(
        ...
        try:
            async with asyncio.timeout(90):
                ...
                # Step 2: Configure session with request, placeholders, and payloads
                update_q = """
            ...
            res_dict = {"success": False, "error": "Caido did not respond within 90 seconds"}
            success = False
        except Exception as e:
    ...
    async def _execute_caido_get_findings_tool(
        ...
        try:
            ...
            if "errors" in data:
                ...
                edges = data.get("data", {}).get("findings", {}).get("edges", [])
                findings = [
                    {
                        "id": e["node"]["id"],
                        "title": e["node"]["title"],
    ...
    async def _execute_caido_set_scope_tool(
        ...
        """
        scope_name = f"airecon-{self.state.active_target or 'scope'}"  # type: ignore[attr-defined]
        variables = {"input": {"name": scope_name, "allowlist": allowlist, "denylist": denylist}}
        ...
        try:
            ...
            if "errors" in data:
                ...
                scope = data.get("data", {}).get("createScope", {}).get("scope", {})
    ...
    async def _execute_code_analysis_tool(
        ...
        active_target = self.state.active_target or "unknown"  # type: ignore[attr-defined]
        ...
        try:
            ...
            res_dict = {
                ...
                "findings": result.get("findings", [])[:50],  # Cap at 50 findings
    ...
    async def _execute_schemathesis_tool(
        ...
        if not schema_url:
            return False, 0.0, {"success": False, "error": "'schema_url' is required."}, None
        ...
        try:
            ...
            stdout = exec_result.get("stdout", "") or exec_result.get("result", "") or ""
    ...
    async def _execute_spawn_agent_tool(
        ...
        target = arguments.get("target", self.state.active_target or "")  # type: ignore[attr-defined]
        ...
        specialist = _RAW_SPECIALIST.lower().strip() if _RAW_SPECIALIST.lower().strip() in _VALID_SPECIALISTS else "exploit"
        ...
        try:
            ...
            agent = AgentLoop(ai=ai, engine=self.engine)  # type: ignore[attr-defined]
            # Limit subagent iterations — focused task, not full recon
            agent._override_max_iterations = 200  # process_message respects this over config
            ...
            if agent._session:
                ...
                if parent_session is not None:
                    for vuln in agent._session.vulnerabilities:
                        if not _is_duplicate_vulnerability(vuln, parent_session.vulnerabilities):
        ...
        self.state.tool_counts["total"] += 1  # type: ignore[attr-defined]
        self.state.tool_counts["subagents"] = self.state.tool_counts.get("subagents", 0) + 1  # type: ignore[attr-defined]
        return success, duration, res_dict, None
    ...
    async def _execute_tool_and_record(
        ...
        # --- SPECIAL TOOL DISPATCH TABLE ---
        # Tools handled by dedicated methods (bypasses the Docker engine executor).
        # The `execute` tool falls through (needs workspace setup first).
        _SPECIAL_HANDLERS = {
            "advanced_fuzz":       self._execute_advanced_fuzz_tool,
            "quick_fuzz":          self._execute_quick_fuzz_tool,
            "deep_fuzz":           self._execute_deep_fuzz_tool,
            "generate_wordlist":   self._execute_generate_wordlist_tool,
            "run_parallel_agents": self._execute_run_parallel_agents_tool,
            "caido_list_requests": self._execute_caido_list_requests_tool,
            "caido_send_request":  self._execute_caido_send_request_tool,
            "caido_automate":      self._execute_caido_automate_tool,
            "caido_get_findings":  self._execute_caido_get_findings_tool,
            "caido_set_scope":     self._execute_caido_set_scope_tool,
            "spawn_agent":         self._execute_spawn_agent_tool,
            "code_analysis":       self._execute_code_analysis_tool,
            "schemathesis_fuzz":   self._execute_schemathesis_tool,
        }
        if tool_name in _SPECIAL_HANDLERS:
            return await _SPECIAL_HANDLERS[tool_name](tool_name, arguments)
        ...
        # Inject a helpful hint for common bash escaping errors (single quotes inside single quotes)
        if not success:
