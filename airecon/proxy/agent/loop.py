...
class AgentLoop(_ValidatorMixin, _FormatterMixin, _WorkspaceMixin, _ExecutorMixin):
    ...
    def __init__(self, ai: AIClient, engine: DockerEngine) -> None:
        self.ai = ai
        self.engine = engine
        ...
        self._blocked_tools: set[str] = set()  # Tools blocked for this agent (e.g. depth control)
        self._override_max_iterations: int | None = None  # If set, overrides config in process_message
        self.pipeline: PipelineEngine | None = None

    @property
    def tools(self) -> list[dict[str, Any]]:
        """Public read-only view of the registered tool definitions."""
    ...
    async def initialize(self) -> None:
        ...
        self._tools = self.engine.tools_to_ollama_format(engine_tools)
        ...
        if self._tools is None:
            self._tools = []
        self._tools.extend(get_tool_definitions())

        # Deduplicate by tool name
        unique: dict[str, dict] = {}
        for t in self._tools:
            unique[t["function"]["name"]] = t
        self._tools = list(unique.values())

        # Remove blocked tools (e.g. spawn_agent blocked for subagents to prevent recursion)
        ...
        # Initialize session: load from env var ID if provided, otherwise create fresh
        import os as _os
        ...
        if _session_id:
            ...
            logger.info(f"Loaded session {_session_id} (target={self._session.target})")
        else:
    ...
    async def process_message(self, user_message: str) -> AsyncIterator[AgentEvent]:
        try:
            ...
            if self.state.active_target:
                ...
                # Update session target (session was pre-created in initialize())
            ...
            while self.state.iteration < self.state.max_iterations:
                ...
                if self.state.iteration > 1 and self.state.iteration % 5 == 0:
                    ...
                    if self._session:
                        ...
                        # so the LLM doesn't need to recall from early conversation turns.
                ...
                # Heavier: adds correlation, vuln chaining, expert testing hints.
                    ...
                    if self._session:
                        ...
                        if s.open_ports or s.technologies:
                            ...
                            if correlations:
                                ...
                                for corr in correlations[:10]:  # Enhanced to show top 10 instead of 5
                                    ...
                                    if vuln_type == "port":
                                        ...
                                        corr_lines.append(f"- [{severity}] Port {port} ({service}): {vuln_str}")
                                        
                                    elif vuln_type == "technology":
                                        tech = corr.get("technology", "?")
                                        vulns = corr.get("vulnerabilities", [])
                                        vuln_str = vulns[0] if vulns else "Multiple issues"
                                        corr_lines.append(f"- [{severity}] Tech {tech}: {vuln_str}")
                                        
                                    elif vuln_type == "technology_cve":
                                        tech = corr.get("technology", "?")
                                        vulns = corr.get("vulnerabilities", [])
                                        vuln_str = vulns[0] if vulns else "Multiple issues"
                                        corr_lines.append(f"- [{severity}] {tech} CVE: {vuln_str}")
                                        
                                    elif vuln_type == "url_path":
                                        path = corr.get("path", "?")
                                        tech = corr.get("technology", "?")
                                        corr_lines.append(f"- [{severity}] Discovered Path '{path}' mapped to {tech}")
                                        
                                    elif vuln_type == "expert_test":
                                        pattern = corr.get("pattern", "?")
                                        desc = corr.get("description", "?")
                                        corr_lines.append(f"- [{severity}] EXPERT TEST ({pattern}): {desc}")
                                        
                                    elif vuln_type == "zeroday_potential":
                                        pattern = corr.get("pattern", "?")
                                        desc = corr.get("description", "?")
                                        corr_lines.append(f"- [{severity}] ZERO-DAY INDICATOR ({pattern}): {desc}")
                                        
                                    elif vuln_type == "business_logic":
                                        pattern = corr.get("pattern", "?")
                                        desc = corr.get("description", "?")
                                        corr_lines.append(f"- [{severity}] BUSINESS LOGIC FLAW ({pattern}): {desc}")
                                        
                                    elif vuln_type == "attack_chain":
                                        name = corr.get("name", "?")
                                        steps = corr.get("steps", [])
                                        steps_str = " -> ".join(steps)
                                        corr_lines.append(f"- [{severity}] ATTACK CHAIN DETECTED ({name}): {steps_str}")
                                        
                                    else:
                                        # Generic fallback
                                        corr_lines.append(f"- [{severity}] Unknown Correlation: {corr}")
                        ...
                        if s.urls and len(s.urls) > 5:
                            ...
                            if expert_patterns:
                                try:
                                    prompt_path = Path(__file__).parent.parent / "prompts" / "testing.txt"
                                    with open(prompt_path, "r") as pf:
                                        expert_template = pf.read()
                                    patterns_str = "\n".join(f"- {p}" for p in expert_patterns)
                                    expert_testing_prompt = "\n\n" + expert_template.replace("{expert_patterns}", patterns_str)
                                except Exception:
                                    expert_testing_prompt = "\n\n[EXPERT TESTING] " + ", ".join(expert_patterns)
                ...
                if self.state.iteration % _ctx_interval == 0:
                    # LLM-based compression first (summarizes old messages, preserves findings)
                    await self.state.compress_with_llm(self.ai, keep_recent=30)
                    # PIN CRITICAL FINDINGS BEFORE TRUNCATION
                    critical_context = self._build_critical_findings_context()
                    # Progressive max_messages: more aggressive as session grows to limit KV cache
                ...
                # Context window — full size by default; reduced to small on VRAM crash retry
                ...
                # On any other error (or if the retry also fails), we stop with an error message.
                for _stream_attempt in range(2):
                  try:
                    ...
                    if _is_vram_crash and _stream_attempt == 0:
                        # First attempt: auto-recover and retry with smaller context
                        logger.warning(
                    ...
                    # Fatal error: not VRAM crash, or second attempt also failed
                    if _is_vram_crash:
                        error_msg = (
                ...
                if has_hallucination_risk and not tool_calls_acc and not has_prior_tool_runs:
                    ...
                    # Don't stop - let the model correct itself
                ...
                # --- TRACK PLANNED TOOLS ---
                # Extract tools mentioned in assistant's text response and track them
                if content_acc:
                    known_tools = []
                ...
                if not tool_calls_acc:
                    ...
                    # Save session on any early text-only exit (with or without TASK_COMPLETE)
                    if self._session:
                        save_session(self._session)
                    yield AgentEvent(type="done", data={})
                    return

                # Deduplicate tool calls (Ollama streaming sometimes emits duplicates per chunk)
                seen_tools = set()
                deduped_tool_calls = []
                ...
                # Check if tools can run in parallel (same category, different targets)
                ...
                for idx, tc in enumerate(tool_calls_acc):
                    ...
                    if tn == "execute":
                        ...
                        if cmd_bin in parallelizable_tools:
                            # Group all parallel tools under a single 'parallel' category
                ...
                async def execute_single_tool(idx: int, tc: dict, args: dict) -> tuple:
                    ...
                    if tn == "execute":
                        self._check_output_dedup(args)  # save old content for post-run merge
                    ...
                    # Post-run: merge new output with pre-existing file content (if any)
                    if tn == "execute":
                ...
                for group_name, group_tasks in tool_groups.items():
                ...
                for idx, tc, args in sequential_only:
                    # Get the result from already-executed parallel task if exists
                ...
                for idx in sorted(all_results.keys()):
                    ...
                    raw_command = (
                        arguments.get("command", "") if tool_name == "execute" else ""
                    )
                    ...
                    if self.pipeline:  # type: ignore[attr-defined]
                        phase_warn = self.pipeline.check_tool_phase_fit(tool_name)  # type: ignore[attr-defined]
                        if phase_warn:
                    ...
                    if success and tool_name == "execute" and self._session:
                        stdout = (
                            result.get("stdout", "") or result.get("result", "") or ""
                        )
                ...
                # Persist session state incrementally after every tool execution
                if self._session:
                    save_session(self._session)
    ...
    # Extensions that support line-based merge+sort (text, not binary/structured)
    _MERGEABLE_EXTENSIONS = frozenset({".txt", ".csv", ".list", ".hosts", ".log", ".out"})

    def _get_command_output_file(self, arguments: dict[str, Any]) -> tuple[str | None, "Path | None"]:
    ...
    def _check_output_dedup(self, arguments: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.info(f"Saved {len(old_lines)} existing lines from '{output_file}' for post-run merge")
        except Exception as e:
            logger.warning(f"Could not save old content of '{output_file}' for merge: {e}")
    ...
    def _build_critical_findings_context(self) -> str:
        ...
        if s.vulnerabilities:
            vuln_titles = [v.get("title", "Unknown") for v in s.vulnerabilities[:10]]
            parts.append(
                f"VULNERABILITIES ({len(s.vulnerabilities)}): {'; '.join(vuln_titles)}"
            )
