...
class AIReconApp(App):
    ...
    async def on_mount(self) -> None:
        ...
        try:
            ...
            if saved:
                ...
                suffix = f"\n  … and {len(saved) - 8} more" if len(saved) > 8 else ""
    ...
    def _show_recon_spinner(self) -> None:
        ...
        try:
            ...
            bar.update(f"[bold #3b82f6]{self._SPINNER_CHARS[0]}[/]  [#8b949e]esc  interrupt[/]")
            bar.styles.height = 1
        except Exception:
            pass
                ...
                if workspace_root in abs_path.parents:
                    ...
                    if len(rel.parts) > 1: # Must be nested under a target folder (target/...)
                        ...
                        while search_path != workspace_root:
                            if any((search_path / folder).is_dir() for folder in ['vulnerabilities', 'output', 'command', 'tools']):
...
"""
    ...
    def on_preview_result(self, result: Any) -> None:
        ...
        if isinstance(result, FilePreviewScreen.Submitted):
            ...
            chat = self.query_one(ChatPanel)
            # Display relative path to user for brevity (Path-based check avoids false positives)
            _abs = Path(abs_path)
            _cwd = Path(cwd)
            display_path = str(_abs.relative_to(_cwd)) if _abs.is_relative_to(_cwd) else abs_path
...
- Click a file in the workspace panel to load it as context for your next prompt."""
    ...
    async def action_cancel_generation(self) -> None:
        ...
        chat = self.query_one("#chat-panel", ChatPanel)
        chat.add_system_message("🛑 User stopped progress (sending stop signal to agent)...")
