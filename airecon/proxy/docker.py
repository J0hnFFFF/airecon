...
class DockerEngine:
    ...
    async def ensure_image(self) -> bool:
        ...
        logger.info(f"Building Docker image '{self.IMAGE_NAME}' — this may take 10-20 minutes...")
    ...
    async def start_container(self, target: str | None = None) -> bool:
        ...
        if target:
            await self.execute(
                f"mkdir -p /workspace/{target}/output /workspace/{target}/tools"
            )

        # Run apt update silently in the background so future "apt install" commands don't fail due to stale repos
    ...
    async def execute(
        ...
        cmd = [
            ...
            # Use PYTHONUNBUFFERED + stdbuf wrapper for unbuffered output instead.
            "-e",
    ...
    async def execute_tool(
        ...
        if explicit_timeout:
            ...
            if timeout != float(explicit_timeout):
                logger.warning(f"Model set timeout={explicit_timeout}s, enforcing minimum {cfg.command_timeout}s")
        else:
