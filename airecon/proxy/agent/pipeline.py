...
class PipelineEngine:
    ...
    # Prevents instant skip-through when prior phase data already satisfies criteria.
    ...
    def _load_phase_prompts(self) -> None:
        ...
        for phase in PipelinePhase:
            ...
            try:
                ...
                logger.warning(f"Failed to load phase prompt for {phase.value}: {e}")
                self._phase_prompts[phase] = self._default_prompt(phase)
    ...
    def should_transition(self) -> bool:
        ...
        # Prevents skipping a phase because prior-phase data already satisfies criteria.
    ...
    def _evaluate_criteria(self, phase: PipelinePhase) -> list[str]:
        ...
        if phase == PipelinePhase.RECON:
            ...
            _recon_extensions = {".txt", ".out", ".nmap", ".csv", ".json", ".xml", ".html", ".log"}
    ...
    def transition(self) -> PipelinePhase | None:
        ...
        logger.info(f"Pipeline transition: {current.value} → {next_phase.value}")
        return next_phase
    ...
    def get_phase_prompt(self) -> str:
        ...
        completed = ", ".join(self.session.completed_phases) if self.session.completed_phases else "none"
    ...
    def check_tool_phase_fit(self, tool_name: str) -> str | None:
        ...
        if current in (PipelinePhase.EXPLOIT, PipelinePhase.REPORT, PipelinePhase.COMPLETE):
            return None
