...
def _run_tui(args) -> None:
    ...
    # If a session ID was requested, pass it via env var so the proxy picks it up
...
def _run_status(args) -> None:
    ...
    async def check():
        ...
        try:
            import subprocess as sp
            result = sp.run(
                ["docker", "ps", "--filter", f"name=airecon-searxng", "--filter", "status=running", "--format", "{{.Status}}"],
                capture_output=True, text=True, timeout=3,
            )
            if result.stdout.strip():
                searxng_configured = bool(cfg.searxng_url)
                searxng_status = f"{G}● running{X}"
                searxng_detail = cfg.searxng_url or f"http://localhost:8080 (auto-managed)"
            else:
...
def _set_config_value(key: str, value: str) -> None:
    ...
    try:
        ...
        logger.warning(f"Could not update config {key}: {e}")
...
def _unload_model_safely():
    ...
    try:
        ...
        # Remove SearXNG container on exit (image is kept, so next start is fast)
        try:
